...
#define MAX_RECORDS \
	0x10000 // should be greater than the total number of functions across the kernel, 64K is completely arbitrary
...
__attribute__((no_instrument_function)) static unsigned long sdi(void)
{
	...
	while (__atomic_exchange_n(&prof_lock, 1, __ATOMIC_ACQUIRE))
...
__attribute__((no_instrument_function)) static bool pred(record_t *a,
							 record_t *b)
{
...
__attribute__((no_instrument_function)) void prof_show(const char *name)
{
	...
	for (size_t i = 1; i < num_records; i++) {
		for (size_t j = i; j > 0 && pred(&records[j - 1], &records[j]);
		     j--) {
	...
	for (size_t i = 0; i < num_records; i++) {
		...
		prints(" calls, avg ");
		printu((records[i].total + (records[i].calls / 2)) /
		       records[i].calls);
		prints(" per call)\n");
	}
